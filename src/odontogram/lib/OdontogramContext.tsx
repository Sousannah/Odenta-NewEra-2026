...
export function OdontogramProvider({
  ...
  // Odenta: the tooth-selection ring defaults to brand-600 rather than the
  // module's stock blue. Still user-adjustable in Settings → Tooth details.
  const [selectionColor, setSelectionColor] = useState<string>("#0077B6");
  ...
  // Odenta: the chart is embedded in a host page, so the class is removed again
  // when the chart unmounts — a dark chart must not outlive the screen it was
  // on and leave the host in dark mode.
  useEffect(() => {
