...
/** Builds the default service list for a reservation from its dental findings. */
export function buildServices(reservation, record) {
  const medicalTeeth = (record?.medical ?? []).filter((entry) => entry.state === "pending");

  const services = [
    {
      id: "svc-treatment",
      name: reservation?.treatment ?? "Treatment",
      kind: "treatment",
      targets: medicalTeeth.length
        ? medicalTeeth.map((entry) => ({
            id: entry.tooth,
            badge: entry.tooth,
