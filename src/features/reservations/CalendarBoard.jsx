      ...
      <div className="relative">
        {hourRange().map((hour) => (
          <button
            ...
            className="group relative block w-full border-b border-slate-100 transition hover:bg-brand-50/50"
            style={{ height: SLOT_HEIGHT }}
          >
        ...
        <div className="relative flex">
          {dentists.map((dentist) => (
            <DentistColumn
              key={dentist.id}
              dentist={dentist}
              appointments={reservations.filter((item) => item.dentistId === dentist.id)}
